...
class Products{
    ...
    private:std::string expire_date;
    ...
    public:Products(std::string Name,ll quantity,double weight,std::string expire_date,double price){
        this->Name=Name;
    ...
    public:std::string getExpire_date(){
...
class Cart{
    ...
    public:void add(std::string Name,ll quantity){
        ...
        if(Items[Name].getExpire_date().size() and std::stoi(Items[Name].getExpire_date())<2025){
...
class E_Commerce_System{
    ...
    public:void RunningSystem(ShippingService&shipping){
        ...
        double customer_balance=customer.getBalance();
        double shipping_recipt=shipping.getAmount();
        
        if(customer.getCart().getProducts().size()==0){
            puts("Cart is Empty");
        }
        if(customer_balance>=shipping_recipt){
        }
        else{
